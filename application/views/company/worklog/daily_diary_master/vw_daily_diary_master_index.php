					...
					<div class="my-auto">
						...
						<nav>
							<ol class="breadcrumb mb-0">
								...
								<li class="breadcrumb-item active" aria-current="page">Daily Diary Master</li>
							</ol>
						</nav>
					</div>
				</div>
					...
					<div class="col-xl-12">
						<div class="card custom-card">
							<div class="card-header">
								<div class="card-title">Daily Diary Master</div>
							</div>
							<div class="card-body">
								<div class="btn-list">

									<a href="create_company_daily_diary_master" class="btn btn-primary btn-wave">
										Add Daily Diary
									</a>
								</div>
							</div>
								...
								<div class="table-responsive">
									<table id="example1" class="table table-bordered text-nowrap w-100">
										...
										<tbody>
											...
											foreach ($daily_diary_list as $row) :
												$no++;
												$daily_diary_id = $row->entity_id;

												$this->db->select('*');
												$this->db->from('participant_daily_diary_attachment_relation');
												$this->db->where('daily_diary_id', $daily_diary_id);
												$attachment1 = $this->db->get();
												$attachment_list = $attachment1->result();
											...
											?>
												<tr>
													<td><?php echo $no; ?></td>
													<td>
														<?php echo $row->diary_date; ?>
													</td>
													<td>
														<?php echo $row->created_date; ?>
													</td>
													<td>
														<?php echo $row->sparrow_employee_name; ?>
													</td>
													<td>
														<?php foreach ($attachment_list as $key => $attachment) { ?>
															<a href="<?php echo base_url('assets/daily_diary/' . $attachment->attachment); ?>" ><?php echo $attachment->attachment; ?></a>
															<br>
														<?php } ?>
													</td>
												<!-- <td>
														<a href="<?php echo site_url('edit_company_daily_diary_master/'.$daily_diary_id); ?>" class="btn btn-sm btn-info" title="Edit"><i class="fas fa-edit"></i></a>
													</td> -->
