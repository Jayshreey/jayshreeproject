...
class Prospect_master_model extends CI_Model{
    ...
    public function get_prospect_tracking_list($prospect_id)
    {
        $this->db->select('*');
        ...
        $this->db->join('nca_activity_type_master','nca_activity_type_master.entity_id = prospect_tracking_master.nca_activity_type_id','inner');
        $this->db->join('nca_activity_result_relation','nca_activity_result_relation.entity_id = prospect_tracking_master.nca_activity_result_id','inner');
        $this->db->where('prospect_tracking_prospect_relation.prospect_id',$prospect_id);
	...
	public function save_prospect($prospect_insert_array)
	{
		$this->db->insert('prospect_master',$prospect_insert_array);
	...
	public function save_subscription($subscription_insert_array)
	{
		$this->db->insert('subscription_master',$subscription_insert_array);
    ...
    {
        $this->db->select('*,concat(sparrow_employee_master.first_name," ",sparrow_employee_master.last_name) as spoc_name');
        $this->db->from('sparrow_employee_master');
    ...
    public function get_prospect_list()
    {
		...
		if ($role_id == 3 || $role_id ==4) {
			$where2 = "(prospect_master.assigned_to = " . $sparrow_employee_id . ")";
		} else {
			$where2 = "(prospect_master.entity_id > 0)";
		}
		...
		$this->db->where('prospect_master.status', 1);
    ...
    public function get_prospect_details_by_id($prospect_id)
    {
        $this->db->select('prospect_master.*,concat(prospect_master.first_name," ",prospect_master.middle_name," ",prospect_master.last_name) as prospect_name,concat(sparrow_employee_master.first_name," ",sparrow_employee_master.last_name) as sparrow_employee_name,state_master.state_name,city_master.city_name');
        $this->db->from('prospect_master');
        $this->db->join('sparrow_employee_master','sparrow_employee_master.entity_id = prospect_master.assigned_to','inner');
        $this->db->join('state_master','state_master.entity_id = prospect_master.state_id','inner');
        $this->db->join('city_master','city_master.entity_id = prospect_master.city_id','inner');
        $this->db->where('prospect_master.entity_id',$prospect_id);
        $prospect_query = $this->db->get();
		$prospect_details = $prospect_query->row();
